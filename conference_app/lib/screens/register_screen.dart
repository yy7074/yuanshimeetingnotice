...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _sendCode() async {
    ...
    try {
      ...
      if (res.statusCode == 201 || res.statusCode == 200) {
        ...
        setState(() { _error = res.body?['message'] ?? 'Failed'; _isLoading = false; });
      }
    } catch (_) {
      // Offline: skip code verification
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      if (res.statusCode == 201 || res.statusCode == 200) {
        ...
        setState(() { _error = res.body?['message'] ?? 'Registration failed'; _isLoading = false; });
      }
    } catch (_) {
      setState(() { _error = _isZh ? '注册失败，请稍后重试' : 'Registration failed'; _isLoading = false; });
    }
  }
              ...
              children: [
                ...
                Text(
                  _step == 0 ? (_isZh ? '我们将发送验证码到您的邮箱' : 'We will send a verification code')
                    ...
                    : (_isZh ? '设置密码并完善个人信息' : 'Set your password and profile'),
  ...
  List<Widget> _buildPasswordStep() => [
    TextField(
      ...
      decoration: InputDecoration(
        labelText: _isZh ? '设置密码（至少8位）' : 'Password (min 8 chars)',
    ...
    SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _isLoading
            ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
            : Text(_isZh ? '完成注册' : 'Register', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      ),
    ),
  ];
}
